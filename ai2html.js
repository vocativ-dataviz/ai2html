...
if (scriptEnvironment == "nyt") {
    var ai2htmlBaseSettings = {
        ...
        max_width: {
            ...
            inputType: "integer",
            possibleValues: "",
            notes: "Blank or any positive number in pixels, but do not write “px” - blank means artboards will set max size, instead it is written to the config file so that the max width can be applied to the template’s container."
        },
        output: {
        ...
        image_output_path: {
            defaultValue: "../public/_assets/",
            includeInSettingsBlock: false,
            includeInConfigFile: false,
            useQuoteMarksInConfigFile: false,
            inputType: "folderPath",
            possibleValues: "",
            notes: "_assets/ for preview default. This is where the image files get written to locally and should be written as if the html_output location is the starting point."
        },
        image_source_path: {
            defaultValue: null,
            includeInSettingsBlock: false,
        ...
        show_completion_dialog_box: {
            ...
            inputType: "trueFalse",
            possibleValues: "",
            notes: "Set this to false if you don't want to see the dialog box confirming completion of the script."
        },
        clickable_link: {
            defaultValue: "",
            includeInSettingsBlock: false,
        ...
        credit: {
            defaultValue: "By The New York Times",
            includeInSettingsBlock: true,
            includeInConfigFile: true,
            useQuoteMarksInConfigFile: true,
            inputType: "text",
        ...
        page_template: {
            defaultValue: "nyt5-article-embed",
            includeInSettingsBlock: true,
            includeInConfigFile: true,
            useQuoteMarksInConfigFile: false,
            inputType: "text",
    ...
    var ai2htmlBaseSettings = {
        ...
        headline: {
            defaultValue: "Ai Graphic Headline",
            includeInSettingsBlock: true,
            includeInConfigFile: true,
            useQuoteMarksInConfigFile: true,
            inputType: "text",
        ...
        leadin: {
            defaultValue: "Introductory text here.",
            includeInSettingsBlock: true,
            includeInConfigFile: true,
            useQuoteMarksInConfigFile: true,
            inputType: "text",
        ...
        notes: {
            defaultValue: "Notes: Text goes here.",
            includeInSettingsBlock: true,
            includeInConfigFile: true,
            useQuoteMarksInConfigFile: true,
            inputType: "text",
        ...
        sources: {
            defaultValue: "Source: Name goes here.",
            includeInSettingsBlock: true,
            includeInConfigFile: true,
            useQuoteMarksInConfigFile: true,
            inputType: "text",
...
var caps = [{
    "ai": "FontCapsOption.NORMALCAPS",
    "html": ""
}, {
    "ai": "FontCapsOption.ALLCAPS",
    "html": "uppercase"
}, {
    "ai": "FontCapsOption.SMALLCAPS",
    "html": "uppercase"
}];
var align = [{
    "ai": "Justification.LEFT",
    "html": ""
}, {
    "ai": "Justification.RIGHT",
    "html": "right"
}, {
    "ai": "Justification.CENTER",
    "html": "center"
}, {
    "ai": "Justification.FULLJUSTIFY",
    "html": "justify"
}, {
    "ai": "Justification.FULLJUSTIFYLASTLINELEFT",
    "html": "justify"
}, {
    "ai": "Justification.FULLJUSTIFYLASTLINECENTER",
    "html": "justify"
}, {
    "ai": "Justification.FULLJUSTIFYLASTLINERIGHT",
    "html": "justify"
}];
var pStyleKeyTags = [
    "aifont",
...
artboardsLefts.sort(function(a, b) {
    return a - b
});
artboardsTops.sort(function(a, b) {
    return a - b
});
        ...
        if (thisFrame.paragraphs[0].contents == "ai2html-settings" || thisFrame.paragraphs[0].contents == "ai2html-text") {
            ...
            for (var p = 1; p < thisFrame.paragraphs.length; p++) {
                ...
                if (paragraphContentIsValid) {
                    ...
                    if (hashKey == "output" && hashValue == "one-file-for-all-artboards") {
                        hashValue = "one-file";
                    };
                    if (hashKey == "output" && hashValue == "one-file-per-artboard") {
                        hashValue = "multiple-files";
                    };
                    if (hashKey == "output" && hashValue == "preview-one-file") {
                        hashValue = "one-file";
                    };
                    if (hashKey == "output" && hashValue == "preview-multiple-files") {
                        hashValue = "multiple-files";
                    };
...
for (setting in docSettings) {
    if (setting in ai2htmlBaseSettings && ai2htmlBaseSettings[setting].includeInConfigFile) {
        var quoteMark = "";
        if (ai2htmlBaseSettings[setting].useQuoteMarksInConfigFile) {
            quoteMark = '"';
        };
...
if (doc.documentColorSpace != "DocumentColorSpace.RGB") {
    ...
    if (docSettings.include_resizer_widths == "yes") {
        ...
        uniqueArtboardWidths.sort(function(a, b) {
            return a - b;
        });
    }
    ...
    for (var abNumber = 0; abNumber < doc.artboards.length; abNumber++) {
        if (artboardsToProcess[abNumber]) {
            ...
            for (var i = 0; i < numHtmlStrands; i++) {
                html[i] = "";
            };
            ...
            for (var i = 0; i < docFrames.length; i++) {
                ...
                if (
                    ...
                    frameInBounds && !frameIsHidden &&
                    // !thisFrame.hidden &&
                    thisFrame.contents != "" &&
                    (
                        docSettings.render_rotated_skewed_text_as == "html" ||
                        (
                            docSettings.render_rotated_skewed_text_as == "image" && !textIsTransformed(thisFrame)
                        )
                    ) &&
            ...
            for (var i = 0; i < pStyleKeys.length; i++) {
                ...
                for (var j = 0; j < pStyleKeyTags.length; j++) {
                    ...
                    if (thisTag == "r" || thisTag == "g" || thisTag == "b") {
                        if (pHash[thisTag].length == 1) {
                            pHash[thisTag] = "0" + pHash[thisTag]
                        };
                    };
                ...
                if (pHash['family'] != defaultFamily) {
                    pStyleCss += "\t\t\t\tfont-family:" + pHash['family'] + ";\r";
                };
                if (pHash['size'] != defaultSize) {
                    pStyleCss += "\t\t\t\tfont-size:" + pHash['size'] + "px;\r";
                };
                if (pHash['leading'] != defaultLeading) {
                    pStyleCss += "\t\t\t\tline-height:" + pHash['leading'] + "px;\r";
                };
                if (pHash['weight'] != "") {
                    pStyleCss += "\t\t\t\tfont-weight:" + pHash['weight'] + ";\r";
                };
                if (pHash['style'] != "") {
                    pStyleCss += "\t\t\t\tfont-style:" + pHash['style'] + ";\r";
                };
                if (pHash['capitalization'] != "") {
                    pStyleCss += "\t\t\t\ttext-transform:" + pHash['capitalization'] + ";\r";
                };
                if (pHash['justification'] != "") {
                    pStyleCss += "\t\t\t\ttext-align:" + pHash['justification'] + ";\r";
                };
                if (pHash['spacebefore'] > 0) {
                    pStyleCss += "\t\t\t\tpadding-top:" + pHash['spacebefore'] + "px;\r";
                };
                if (pHash['spaceafter'] > 0) {
                    pStyleCss += "\t\t\t\tpadding-bottom:" + pHash['spaceafter'] + "px;\r";
                };
                if (pHash['tracking'] != 0) {
                    pStyleCss += "\t\t\t\tletter-spacing:" + pHash['tracking'] + "em;\r";
                };
                if (pHash['opacity'] != 1.0) {
                    pStyleCss += "\t\t\t\tfilter: alpha(opacity=" + (pHash['opacity'] * 100) + ");\r";
                };
                if (pHash['opacity'] != 1.0) {
                    pStyleCss += "\t\t\t\t-ms-filter:'progid:DXImageTransform.Microsoft.Alpha(Opacity=" + (pHash['opacity'] * 100) + ")';\r";
                };
                if (pHash['opacity'] != 1.0) {
                    pStyleCss += "\t\t\t\topacity:" + pHash['opacity'] + ";\r";
                };
            ...
            for (var i = 0; i < selectFrames.length; i++) {
                ...
                if (textIsTransformed(thisFrame)) {
                    ...
                    var transform = "matrix(" + round(mat.mValueA, pctPrecision) + ',' + round(-1 * mat.mValueB, pctPrecision) + ',' + round(-1 * mat.mValueC, pctPrecision) + ',' + round(mat.mValueD, pctPrecision) + ',' + round(t_trans_x, pctPrecision) + ',' + round(-t_trans_y, pctPrecision) + ') ' + "scaleX(" + round(t_scale_x, pctPrecision) + ") " + "scaleY(" + round(t_scale_y, pctPrecision) + ")";
            ...
            if (docSettings.output == "multiple-files") {
                ...
                if (customCssBlocks > 0) {
                    textForFile += customCss;
                };
                textForFile += responsiveTextScoop;
                if (customHtml.length > 0) {
                    textForFile += customHtml;
                };
                if (customJs.length > 0) {
                    textForFile += customJs;
                };
                if (responsiveJs.length > 0) {
...
if (customCssBlocks == 1) {
    feedback.push(customCssBlocks + " custom CSS block was added.")
};
if (customCssBlocks > 1) {
    feedback.push(customCssBlocks + " custom CSS blocks were added.")
};
if (customHtmlBlocks == 1) {
    feedback.push(customHtmlBlocks + " custom HTML block was added.")
};
if (customHtmlBlocks > 1) {
    feedback.push(customHtmlBlocks + " custom HTML blocks were added.")
};
if (customJsBlocks == 1) {
    feedback.push(customJsBlocks + " custom JS block was added.")
};
if (customJsBlocks > 1) {
    feedback.push(customJsBlocks + " custom JS blocks were added.")
};
